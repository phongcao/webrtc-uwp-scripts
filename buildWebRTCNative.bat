...
:buildNativeLibs
    CALL:print %warning% "Building WebRTC native and uwp libs"
	!msVS_Path!\MSBuild\15.0\Bin\msbuild.exe webrtc\windows\solutions\WebRtc.sln /property:Platform=x86;Configuration=Release
	!msVS_Path!\MSBuild\15.0\Bin\msbuild.exe webrtc\windows\solutions\WebRtc.sln /property:Platform=x64;Configuration=Release
	!msVS_Path!\MSBuild\15.0\Bin\msbuild.exe webrtc\windows\solutions\WebRtc.sln /property:Platform=x86;Configuration=Debug
	!msVS_Path!\MSBuild\15.0\Bin\msbuild.exe webrtc\windows\solutions\WebRtc.sln /property:Platform=x64;Configuration=Debug

    IF ERRORLEVEL 1 CALL:error 1 "Building WebRTC native and uwp libs has failed"
GOTO:EOF
...
:done
ECHO.
CALL:print %info% "Success: WebRTC native and UWP libraries were built successfully. Find them in Output/ folder."
ECHO.
SET endTime=%time%
CALL:showTime
:end
